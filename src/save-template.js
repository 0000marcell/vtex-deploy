...
module.exports = async function(opts, cb) {
  ...
  try {
    return request({
      ...
      let success = false;
      if(error) {
        log && console.error(`Template ${opts.name} was not saved: `, error);
      }
      if(response.statusCode === 200 && !/originalMessage/.test(body)) {
        log && console.log(`* Template ${opts.name} was saved on ${opts.store}`);
        success = true;
      } else {
        log && console.error(`* Template ${opts.name} was not saved!`);
    ...
    log && console.error(`Template ${opts.name} was not saved error: ${err}`); 
    if(typeof cb == 'function'){
      cb(null);
    }
  }
